...
pub(super) fn derive_packet(item: TokenStream) -> Result<TokenStream> {
    ...
    let side = if let Some(side_attr) = packet_attr.side {
        ...
        parse_quote!(::protocol_lib::types::PacketSide::Clientbound)
    } else if name_str.to_lowercase().ends_with("c2s") {
        parse_quote!(::protocol_lib::types::PacketSide::Serverbound)
    } else {
        return Err(Error::new(
            packet_attr.span,
    ...
    Ok(quote! {
        ...
        #where_clause
        {
            ...
            const SIDE: ::protocol_lib::types::PacketSide = #side;
            const STATE: ::protocol_lib::types::PacketState = #state;
        }
    })
}
